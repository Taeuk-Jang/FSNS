...
for repeat in range(args.repeat):
        ...
        optimizerH = optim.Adam(H.parameters(), lr=args.lr_h, weight_decay=1e-4)
        optimizerC = optim.Adam(C.parameters(), lr=args.lr_c, weight_decay=1e-4)
        optimizerP = optim.Adam(P.parameters(), lr=args.lr_p, weight_decay=1e-4)

        celoss = nn.functional.binary_cross_entropy
        svmloss = svm_loss
        ...
        for epoch in range(epochs+1):        
            ...
            if epoch ==0:
                ...
                for x, a, y in trainloader:                        
                    ...
                    y_onehot = one_hot_embedding(y, 2).to(device)
                    ##############################
                    ####### train Classifier #####
                    ##############################
                    ...
                    latent = H(x)
                    pred = C(latent.detach())

                    loss_C = max_margin_loss(y_onehot, pred, alpha, mu, lamda, device)
            ...
            for x, a, y in trainloader:
                x, a, y = x.to(device), a.to(device), y.to(device)
                a_onehot = one_hot_embedding(a, num_sens).to(device)
                y_onehot = one_hot_embedding(y, 2).to(device)
                ...
                mm_loss = max_margin_loss(y_onehot, pred, alpha, mu, lamda, device)
                ar_loss = cross_entropy(a_r, sens)

                loss_H = mm_loss + eta * ar_loss
                loss_H.backward()
                ...
                acc += sum(pred.argmax(-1) == y)
                cnt += len(y)   

            acc /= cnt

            if isnan(loss_C.item()) or isnan(loss_H.item()) or isnan(loss_P.item()):
                break

            if epoch % FIG_EPOCH == 0 and VIEW_FIG:
                latent = H(x)
            ...
            if epoch % TEST_EPOCH == 0 and VIEW_EVAL:
        ...
        tpr_overall_hist.append((tpr_overall_value))
        tpr_priv_hist.append((tpr_priv_value))
        tpr_unpriv_hist.append((tpr_unpriv_value))
        eq_opp_hist.append((eq_opp_value))

        fpr_overall_hist.append((fpr_overall_value))
        fpr_priv_hist.append((fpr_priv_value))
        fpr_unpriv_hist.append((fpr_unpriv_value))
        fpr_diff_hist.append((fpr_diff_value))

        acc_overall_hist.append( (acc_overall_value))
        acc_priv_hist.append((acc_priv_value))
        acc_unpriv_hist.append((acc_unpriv_value))
        acc_diff_hist.append((acc_diff_value))

        balanced_hist.append((balanced))
        dis_impact_hist.append((dis_impact))
        theil_hist.append((theil))
